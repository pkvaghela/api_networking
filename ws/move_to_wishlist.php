<?php 
    /*
        ...
        how to call : http://localhost/flutter/ws/move_to_wishlist.php?usersid=3&productid=1
        output : 
  ...
  
{
    $sql = "delete from cart where billid=0 and usersid = {$input['usersid']} and productid = {$input['productid']}";
     ...
     mysqli_query($link,$sql) or ReturnErorr(null,__LINE__);
   
    $sql = "select id from wishlist where productid={$input['productid']} and usersid={$input['usersid']}";
    $wishlist = mysqli_query($link,$sql) or ReturnErorr(nul,__LINE__);
    ...
    array_push($response,array("message"=>"product move to wishlist"));
